...
class CenterViewFlowLayout: UICollectionViewFlowLayout {
    ...
    func frameForItemAtIndexPath(indexPath: NSIndexPath) -> CGRect {
        ...
        let pageMarginX = CGFloat(25.0)//(canvasSize.width - CGFloat(columnCount) * self.itemSize.width - (columnCount > 1 ? CGFloat(columnCount - 1) * self.minimumLineSpacing : 0)) / 2.0
